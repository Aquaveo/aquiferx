...
Key terms used throughout AquiferX and this documentation.

---

**Aquifer**
:   A subsurface layer of rock or sediment that holds and transmits groundwater. In AquiferX, aquifers are spatial subdivisions within a region, each defined by a boundary polygon.
...
:   A system that defines how spatial coordinates map to locations on Earth. AquiferX works internally in WGS 84 (EPSG:4326) and automatically reprojects data from other CRS definitions.

**Data Type**
:   A category of measurement in AquiferX (e.g., water table elevation, salinity). Each data type has a code, name, and unit. Data is stored in separate files per type (`data_{code}.csv`).

**ELM (Extreme Learning Machine)**
:   A single-hidden-layer neural network where input weights are randomly assigned and only the output weights are trained. Used in AquiferX for imputing missing groundwater data based on climate features.

**GLDAS (Global Land Data Assimilation System)**
:   A NASA dataset that provides global, gridded climate and hydrology variables including soil moisture. AquiferX uses GLDAS soil moisture as input features for the ELM imputation model.
...
**Haversine Distance**
:   A formula for computing the great-circle distance between two points on a sphere, given their latitude and longitude. Used throughout AquiferX for geographic distance calculations.
...
**MAvg (Moving Average)**
:   A smoothing technique that replaces each value with a weighted average of nearby values. In AquiferX, this uses a Nadaraya-Watson kernel regression with a Gaussian kernel.
...
**Region**
:   The top-level geographic unit in AquiferX. A region is defined by a boundary polygon and contains aquifers, wells, and measurements.
...
:   The elevation of the top of the saturated zone in an unconfined aquifer, measured from a reference datum. The default and primary data type in AquiferX.
